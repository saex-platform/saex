...
int wmain(int argc,wchar_t** argv) {
    if (argc!=6) return 2; // Full, empty, readonly fixtures; proxy, codec and ASI pins.
    try {
        ...
        auto run=[&](int mode=0) {
            ...
            instance.name="Local\\SAEX.CwdQueryFixture.v1"; instance.name_rva=exe.symbol("instance_name");
            ...
            if(mode==1)query.iat_rva=0;
            if(mode==2)query.helper_rva=0;
            if(mode==3)query.function.highlow_mask=0;
            if(mode==4)query.return_prefix[15]^=std::byte{1};
            if(mode==5)query.implementation.prefix[0]^=std::byte{1};
            if(mode==6)query.iat_rva+=4;
            if(mode==10)query.expected_directory="C:\\wrong-directory";
            LoaderLimits limits{};if(mode==7)limits.events=1;
            if(mode==9){
                LoaderTrace foreign{};std::thread worker([&]{foreign=LoaderObservation::run_cwd_query(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query);});
                worker.join();require(foreign.reason=="loader_owner_thread" && !child.stopped(),"foreign owner");
            }
            const auto result=[&]() -> LoaderTrace {
                if(mode==8)return LoaderObservation::run_cwd_acquire(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire);
                return LoaderObservation::run_cwd_query(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,limits);
            }();
            std::cout<<"case="<<cases<<" mode="<<mode<<" reason="<<result.reason<<" stage="<<result.cwd_query.stage<<'\n';
        ...
        const auto normal=run();const auto& n=normal.cwd_query;
        require(normal.reason=="cwd_query_verified" && normal.cwd_acquire.verified && n.verified && n.continued && n.wrapper_call_reached && n.helper_entered &&
            n.branch_reached && n.query_path_reached && n.call_reached && n.function_entered && n.function_returned && n.shape_valid && n.frame_valid && n.stack_valid &&
            n.seh_preserved && n.prior_record_preserved && n.caller_preserved && n.root_preserved && n.localisation_preserved && n.lock_preserved && n.cookie_preserved &&
            n.guard_preserved && n.buffer_read && n.path_verified && n.returned_length && n.stage==7,"normal incomplete");
        for(int mode:{1,2,3})require(run(mode).reason=="cwd_query_invalid_spec","invalid spec");
        for(int mode:{4,5,6}){const auto bad=run(mode);require(bad.reason=="cwd_query_precondition_shape" && !bad.cwd_query.continued && bad.cwd_acquire.verified,"bad shape advanced");}
        require(run(7).reason=="loader_event_limit","event budget");
        const auto legacy=run(8);require(legacy.reason=="cwd_acquire_verified" && legacy.cwd_acquire.verified && !legacy.cwd_query.armed,"legacy advanced");
        require(run(9).cwd_query.verified,"owner recovery");
        const auto mismatch=run(10);require(mismatch.reason=="cwd_query_directory_mismatch" && mismatch.cwd_query.function_returned && !mismatch.cwd_query.verified,"different cwd accepted");
        {
            const auto held=CreateEventA(nullptr,TRUE,FALSE,"Local\\SAEX.CwdQueryFixture.v1");require(held,"held event");
            const auto v=run();const auto state=WaitForSingleObject(held,0);CloseHandle(held);
            require(v.reason=="instance_existing_detected" && !v.cwd_query.armed && state==WAIT_TIMEOUT,"existing instance advanced");
        }
        for(const auto path:{argv[1]}) {
            ...
            require(waited==WAIT_OBJECT_0 && read && code==92 && markers.exists(L".event-app-called"),"post-acquire canary insensitive");
        }
        const auto scenarios=cases;DWORD before{},after{};
        require(GetProcessHandleCount(GetCurrentProcess(),&before)!=0,"handles before");
        for (unsigned i=0;i<12;++i) require(run().cwd_query.verified,"warm boundary");
        require(GetProcessHandleCount(GetCurrentProcess(),&after)!=0 && before==after,"handle leak");
        std::cout<<"PASS cwd query: "<<scenarios<<" scenarios, one post-query canary control and 12 warm cycles; handles "<<before<<" -> "<<after<<'\n';return 0;
