...
int wmain(int argc,wchar_t** argv) {
    ...
    try {
        ...
        auto run=[&](int mode=0) {
            ...
            instance.name="Local\\SAEX.ApplicationRoutingFixture.v1"; instance.name_rva=exe.symbol("instance_name");
            ...
            if (mode==1) routing.detour_rva=0;
            if (mode==2) routing.index_rva=dispatch.application.target_rva;
            if (mode==3) routing.indices[24]=std::byte{6};
            if (mode==4) routing.indices[0]=std::byte{9};
            if (mode==5) routing.initializer.target_prefix[15]^=std::byte{1};
            if (mode==6) routing.target_rvas[0]++;
            LoaderLimits limits{};if (mode==7) limits.events=1;
            if (mode==9) {
                LoaderTrace foreign{};std::thread worker([&] { foreign=LoaderObservation::run_application_routing(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing); });
                worker.join();require(foreign.reason=="loader_owner_thread" && !child.stopped(),"foreign owner");
            }
            const auto result=[&]() -> LoaderTrace {
                if (mode==8) return LoaderObservation::run_event_dispatch(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch);
                return LoaderObservation::run_application_routing(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,limits);
            }();
            std::cout<<"case="<<cases<<" mode="<<mode<<" reason="<<result.reason<<" stage="<<result.application_routing.stage<<'\n';
        ...
        const auto normal=run();const auto& n=normal.application_routing;
        require(normal.reason=="application_routing_boundary_verified" && normal.event_dispatch.verified && normal.instance_startup.verified && n.verified &&
            n.continued && n.entry_reached && n.detour_reached && n.indirect_reached && n.initializer_call_reached && n.shape_valid && n.frame_valid && n.stack_preserved && n.stage==4 && n.selected_index==5 && n.selected_target==n.stop_address,"normal incomplete");
        for (int mode:{1,2,3}) require(run(mode).reason=="application_routing_invalid_spec","invalid spec");
        for (int mode:{4,5,6}) {
            const auto bad=run(mode);require(bad.reason=="application_routing_precondition_shape" && !bad.application_routing.continued && bad.event_dispatch.verified,"bad shape advanced");
        }
        require(run(7).reason=="loader_event_limit","event budget");
        const auto legacy=run(8);require(legacy.reason=="event_dispatch_boundary_verified" && legacy.event_dispatch.verified && !legacy.application_routing.armed,"legacy advanced");
        require(run(9).application_routing.verified,"owner recovery");
        {
            const auto held=CreateEventA(nullptr,TRUE,FALSE,"Local\\SAEX.ApplicationRoutingFixture.v1");require(held,"held event");
            const auto v=run();const auto state=WaitForSingleObject(held,0);CloseHandle(held);
            require(v.reason=="instance_existing_detected" && !v.application_routing.armed && state==WAIT_TIMEOUT,"existing instance advanced");
        }
        {
            ...
            require(waited==WAIT_OBJECT_0 && read && code==92 && markers.exists(L".event-app-called"),"initializer canary insensitive");
        }
        const auto scenarios=cases;DWORD before{},after{};
        require(GetProcessHandleCount(GetCurrentProcess(),&before)!=0,"handles before");
        for (unsigned i=0;i<12;++i) require(run().application_routing.verified,"warm boundary");
        require(GetProcessHandleCount(GetCurrentProcess(),&after)!=0 && before==after,"handle leak");
        std::cout<<"PASS application routing: "<<scenarios<<" scenarios, initializer canary control and 12 warm cycles; handles "<<before<<" -> "<<after<<'\n';return 0;
