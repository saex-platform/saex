...
int wmain(int argc,wchar_t** argv) {
    ...
    try {
        ...
        auto run=[&](int mode=0) {
            ...
            instance.name="Local\\SAEX.GamePreludeFixture.v1"; instance.name_rva=exe.symbol("instance_name");
            ...
            if (mode==1) prelude.flags_rva=0;
            if (mode==2) prelude.empty.target_prefix[0]=std::byte{0x90};
            if (mode==3) prelude.localisation.target_prefix[0]^=std::byte{1};
            if (mode==4) prelude.stop_prefix[15]^=std::byte{1};
            if (mode==5) {
                prelude.flags_rva++;
                require(game_prelude_body(prelude,region_body),"mutated body");std::copy_n(region_body.begin(),16,prelude.localisation.target_prefix.begin());
            }
            if (mode==6) prelude.stop_prefix[6]^=std::byte{1};
            LoaderLimits limits{};if (mode==7) limits.events=1;
            if (mode==9) {
                LoaderTrace foreign{};std::thread worker([&] { foreign=LoaderObservation::run_game_prelude(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude); });
                worker.join();require(foreign.reason=="loader_owner_thread" && !child.stopped(),"foreign owner");
            }
            const auto result=[&]() -> LoaderTrace {
                if (mode==8) return LoaderObservation::run_application_routing(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing);
                return LoaderObservation::run_game_prelude(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,limits);
            }();
            std::cout<<"case="<<cases<<" mode="<<mode<<" reason="<<result.reason<<" stage="<<result.game_prelude.stage<<'\n';
        ...
        const auto normal=run();const auto& n=normal.game_prelude;
        require(normal.reason=="game_prelude_verified" && normal.application_routing.verified && normal.instance_startup.verified && n.verified &&
            n.continued && n.initializer_entry_reached && n.empty_entry_reached && n.empty_returned && n.localisation_entry_reached && n.localisation_returned && n.shape_valid && n.frame_valid && n.stack_preserved && n.stage==5 && n.flags_read && n.flags_valid,"normal incomplete");
        require(n.flags_before[4]==std::byte{7} && n.flags_before[5]==std::byte{8} && n.flags_before[6]==std::byte{9} && n.flags_after[4]==std::byte{1} && n.flags_after[5]==std::byte{} && n.flags_after[6]==std::byte{},"native writes missing");
        for (int mode:{1,2,3}) require(run(mode).reason=="game_prelude_invalid_spec","invalid spec");
        for (int mode:{4,5,6}) {
            const auto bad=run(mode);require(bad.reason=="game_prelude_precondition_shape" && !bad.game_prelude.continued && bad.application_routing.verified,"bad shape advanced");
        }
        require(run(7).reason=="loader_event_limit","event budget");
        const auto legacy=run(8);require(legacy.reason=="application_routing_boundary_verified" && legacy.application_routing.verified && !legacy.game_prelude.armed,"legacy advanced");
        require(run(9).game_prelude.verified,"owner recovery");
        {
            const auto held=CreateEventA(nullptr,TRUE,FALSE,"Local\\SAEX.GamePreludeFixture.v1");require(held,"held event");
            const auto v=run();const auto state=WaitForSingleObject(held,0);CloseHandle(held);
            require(v.reason=="instance_existing_detected" && !v.game_prelude.armed && state==WAIT_TIMEOUT,"existing instance advanced");
        }
        {
            ...
            require(waited==WAIT_OBJECT_0 && read && code==92 && markers.exists(L".event-app-called"),"file-manager canary insensitive");
        }
        const auto scenarios=cases;DWORD before{},after{};
        require(GetProcessHandleCount(GetCurrentProcess(),&before)!=0,"handles before");
        for (unsigned i=0;i<12;++i) require(run().game_prelude.verified,"warm boundary");
        require(GetProcessHandleCount(GetCurrentProcess(),&after)!=0 && before==after,"handle leak");
        std::cout<<"PASS game prelude: "<<scenarios<<" scenarios, file-manager canary control and 12 warm cycles; handles "<<before<<" -> "<<after<<'\n';return 0;
