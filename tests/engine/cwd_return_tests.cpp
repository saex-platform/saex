...
int wmain(int argc,wchar_t** argv) {
    ...
    try {
        ...
        auto run=[&](int mode=0) {
            ...
            instance.name="Local\\SAEX.CwdReturnFixture.v1"; instance.name_rva=exe.symbol("instance_name");
            ...
            if(mode==1)completion.checker_rva=0;
            if(mode==2)completion.checker_rva=query.helper_rva;
            if(mode==3)completion.checker_rva=UINT32_MAX;
            if(mode==4)completion.return_prefix[15]^=std::byte{1};
            if(mode==5)completion.checker_rva+=1;
            ...
            if(mode==9){
                LoaderTrace foreign{};std::thread worker([&]{foreign=LoaderObservation::run_cwd_return(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion);});
            ...
            const auto result=[&]() -> LoaderTrace {
                ...
                return LoaderObservation::run_cwd_return(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion,limits);
            }();
            std::cout<<"case="<<cases<<" mode="<<mode<<" reason="<<result.reason<<" stage="<<result.cwd_return.stage<<'\n';
        ...
        const auto normal=run();const auto& n=normal.cwd_return;
        require(normal.reason=="cwd_return_verified" && normal.cwd_copy.verified && n.verified && n.continued && n.arguments_cleaned &&
            n.cookie_call_reached && n.checker_entered && n.cookie_matched && n.checker_returned && n.helper_returned && n.shape_valid && n.frame_valid &&
            n.stack_preserved && n.source_snapshot_preserved && n.source_retired && n.destination_preserved && n.seh_preserved && n.prior_record_preserved && n.caller_preserved &&
            n.lock_preserved && n.cookie_preserved && n.guard_preserved && n.localisation_preserved && n.last_error_preserved && n.stage==6,"normal incomplete");
        for(int mode:{1,2,3})require(run(mode).reason=="cwd_return_invalid_spec","invalid spec");
        for(int mode:{4,5}){const auto bad=run(mode);require(bad.reason=="cwd_return_precondition_shape" && !bad.cwd_return.continued && bad.cwd_copy.verified,"bad shape advanced");}
        const auto query_shape=run(6);require(query_shape.reason=="cwd_query_precondition_shape" && !query_shape.cwd_return.armed,"query shape advanced");
        require(run(7).reason=="loader_event_limit","event budget");
        const auto legacy=run(8);require(legacy.reason=="cwd_query_verified" && legacy.cwd_query.verified && !legacy.cwd_return.armed,"legacy query advanced");
        require(run(9).cwd_return.verified,"owner recovery");
        const auto mismatch=run(10);require(mismatch.reason=="cwd_query_directory_mismatch" && mismatch.cwd_query.function_returned && !mismatch.cwd_return.armed,"different cwd copied");
        const auto acquire_only=run(11);require(acquire_only.reason=="cwd_acquire_verified" && !acquire_only.cwd_query.armed && !acquire_only.cwd_return.armed,"legacy acquire advanced");
        const auto copy_only=run(12);require(copy_only.reason=="cwd_copy_verified" && copy_only.cwd_copy.verified && !copy_only.cwd_return.armed,"legacy copy advanced");
        {
            const auto held=CreateEventA(nullptr,TRUE,FALSE,"Local\\SAEX.CwdReturnFixture.v1");require(held,"held event");
            const auto v=run();const auto state=WaitForSingleObject(held,0);CloseHandle(held);
            require(v.reason=="instance_existing_detected" && !v.cwd_return.armed && state==WAIT_TIMEOUT,"existing instance advanced");
        }
        for(const auto path:{argv[1]}) {
            ...
            require(waited==WAIT_OBJECT_0 && read && code==92 && markers.exists(L".event-app-called"),"post-helper canary insensitive");
        }
        const auto scenarios=cases;DWORD before{},after{};
        require(GetProcessHandleCount(GetCurrentProcess(),&before)!=0,"handles before");
        for (unsigned i=0;i<12;++i) require(run().cwd_return.verified,"warm boundary");
        require(GetProcessHandleCount(GetCurrentProcess(),&after)!=0 && before==after,"handle leak");
        std::cout<<"PASS cwd return: "<<scenarios<<" scenarios, one post-helper canary control and 12 warm cycles; handles "<<before<<" -> "<<after<<'\n';return 0;
