...
int wmain(int argc,wchar_t** argv) {
    ...
    try {
        ...
        unsigned cases{};
        auto run=[&](int mode=0) {
            ++cases; const auto path=mode==13?argv[6]:mode==14?argv[7]:mode==15?argv[8]:argv[1]; Markers markers(path); Fixture exe(path);
            ...
            instance.name="Local\\SAEX.CdStreamAllocationFixture.v1"; instance.name_rva=exe.symbol("instance_name");
            ...
            if(mode==1)allocation.malloc_rva=0;
            if(mode==2)allocation.scope_rva+=1;
            if(mode==3)allocation.heap_handle_rva=allocation.heap_mode_rva;
            if(mode==4)allocation.scope_cleanup_rva+=1;
            if(mode==5)allocation.nh_rva+=1;
            if(mode==6)allocation.function.prefix[0]^=std::byte{1};
            if(mode==7)allocation.iat_rva+=4;
            if(mode==8)allocation.function.rva+=1;
            if(mode==9)query.expected_directory="C:\\wrong-directory";
            LoaderLimits limits{160};if(mode==10)limits.events=1;
            if(mode==16)limits.events=161;
            if(mode==17)limits.events=129;
            if(mode==12){
                auto foreign=std::make_unique<LoaderTrace>();std::thread worker([&]{*foreign=LoaderObservation::run_cd_stream_allocation(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion,ready,stream_spec,disk,allocation);});
            ...
            *result=[&]() -> LoaderTrace {
                if(mode==11)return LoaderObservation::run_cd_stream_disk(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion,ready,stream_spec,disk);
                if(mode==17)return LoaderObservation::run_cd_stream_disk(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion,ready,stream_spec,disk,limits);
                return LoaderObservation::run_cd_stream_allocation(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion,ready,stream_spec,disk,allocation,limits);
            }();
            std::cout<<"case="<<cases<<" mode="<<mode<<" reason="<<result->reason<<" stage="<<result->cd_stream_allocation.stage<<'\n';
        ...
        const auto normal=run();const auto& n=normal->cd_stream_allocation;
        require(normal->reason=="cd_stream_allocation_verified" && normal->cd_stream_disk.verified && n.verified && n.stage==10 &&
            n.request_valid && n.frame_valid && n.arguments_valid && n.globals_preserved && n.branch_verified &&
            n.api_returned && n.allocation_succeeded && n.block_valid && n.metadata_valid && n.content_preserved &&
            n.parent_preserved && n.seh_preserved && n.prior_record_preserved,"normal incomplete");
        require(n.block.raw && n.metadata_after==n.block.raw && n.block_before_hash==n.block_after_hash &&
            n.block.aligned%normal->cd_stream_disk.geometry.bytes_per_sector==0,"block result");
        for(int mode:{1,2,3})require(run(mode)->reason=="cd_stream_allocation_invalid_spec","invalid allocation spec");
        for(int mode:{4,5,6,7,8}){const auto v=run(mode);require(v->reason=="cd_stream_allocation_precondition_shape" && !v->cd_stream_allocation.continued && v->cd_stream_disk.verified,"bad shape advanced");}
        require(run(9)->reason=="cwd_query_directory_mismatch","wrong cwd");
        require(run(10)->reason=="loader_event_limit","event budget");
        const auto legacy=run(11);require(legacy->reason=="cd_stream_disk_verified" && !legacy->cd_stream_allocation.armed,"legacy advanced");
        require(run(12)->cd_stream_allocation.verified,"owner recovery");
        const auto sbh=run(13);require(sbh->cd_stream_allocation.verified && sbh->cd_stream_allocation.globals[3]==3,"SBH bypass");
        for(int mode:{14,15}){const auto v=run(mode);require(v->reason=="cd_stream_allocation_request_rejected" && !v->cd_stream_allocation.continued,"unsafe heap branch advanced");}
        ...
        {
            const auto held=CreateEventA(nullptr,TRUE,FALSE,"Local\\SAEX.CdStreamAllocationFixture.v1");require(held,"held event");
            const auto v=run();const auto state=WaitForSingleObject(held,0);CloseHandle(held);
            require(v->reason=="instance_existing_detected" && !v->cd_stream_allocation.armed && state==WAIT_TIMEOUT,"existing instance advanced");
        }
        for(const auto path:{argv[1],argv[6]}) {
            ...
            require(waited==WAIT_OBJECT_0 && read && code==92 && markers.exists(L".event-app-called"),"post-allocation canary insensitive");
        }
        const auto scenarios=cases;DWORD before{},after{};
        require(GetProcessHandleCount(GetCurrentProcess(),&before)!=0,"handles before");
        for (unsigned i=0;i<12;++i) require(run()->cd_stream_allocation.verified,"warm boundary");
        require(GetProcessHandleCount(GetCurrentProcess(),&after)!=0 && before==after,"handle leak");
        std::cout<<"PASS cd stream allocation: "<<scenarios<<" scenarios, two post-allocation canary controls and 12 warm cycles; handles "<<before<<" -> "<<after<<'\n';return 0;
