...
int wmain(int argc,wchar_t** argv) {
    const bool control=argc==3 && std::wstring_view(argv[1])==L"--copy-control";
    if (argc!=6 && !control) return 2; // Copy fixture; proxy, codec and ASI pins.
    try {
        if(control) {
            // DLL startup also emits markers: own the whole child lifetime, even for
            // the standalone body test. A second run proves cleanup is repeatable.
            for(unsigned repeat=0;repeat<2;++repeat) {
                Markers markers(argv[2]);STARTUPINFOW startup{};startup.cb=sizeof(startup);
                startup.dwFlags=STARTF_USESHOWWINDOW;startup.wShowWindow=SW_HIDE;
                PROCESS_INFORMATION process{};std::wstring command=L"\""+std::wstring(argv[2])+L"\" --copy-control v1 run";
                require(CreateProcessW(argv[2],command.data(),nullptr,nullptr,FALSE,CREATE_NO_WINDOW,nullptr,nullptr,&startup,&process)!=0,"body control create");
                ...
                require(waited==WAIT_OBJECT_0 && read && code==0,"body control failed");
                require(markers.exists(L".dll-entered") && markers.exists(L".proxy-entered") && !markers.exists(L".event-app-called"),"body control path");
            }
            std::cout<<"PASS cwd copier body: 1984 length/alignment cases; source and destination guards preserved; two repeats with owned marker cleanup\n";
            return 0;
        }
        ...
        auto run=[&](int mode=0) {
            ...
            instance.name="Local\\SAEX.CwdCopyFixture.v1"; instance.name_rva=exe.symbol("instance_name");
            ...
            if(mode==1)copy.copier_rva=0;
            if(mode==2)copy.copier_rva=query.helper_rva;
            if(mode==3)copy.copier_rva=UINT32_MAX;
            if(mode==4)copy.return_prefix[15]^=std::byte{1};
            if(mode==5)copy.copier_rva+=1;
            ...
            if(mode==9){
                LoaderTrace foreign{};std::thread worker([&]{foreign=LoaderObservation::run_cwd_copy(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy);});
            ...
            const auto result=[&]() -> LoaderTrace {
                ...
                return LoaderObservation::run_cwd_copy(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,limits);
            }();
            std::cout<<"case="<<cases<<" mode="<<mode<<" reason="<<result.reason<<" stage="<<result.cwd_copy.stage<<'\n';
        ...
        const auto normal=run();const auto& n=normal.cwd_copy;
        require(normal.reason=="cwd_copy_verified" && normal.cwd_query.verified && n.verified && n.continued && n.pointer_branch_reached &&
            n.capacity_branch_reached && n.call_reached && n.function_entered && n.function_returned && n.shape_valid && n.frame_valid && n.arguments_valid &&
            n.stack_preserved && n.source_preserved && n.destination_valid && n.seh_preserved && n.prior_record_preserved && n.caller_preserved &&
            n.lock_preserved && n.cookie_preserved && n.guard_preserved && n.localisation_preserved && n.last_error_preserved && n.copied &&
            n.copied_bytes==normal.cwd_query.returned_length+1 && n.stage==5,"normal incomplete");
        require(std::equal(normal.cwd_query.directory.begin(),normal.cwd_query.directory.begin()+n.copied_bytes,n.destination_after.begin()+4),"actual copied content");
        for(int mode:{1,2,3})require(run(mode).reason=="cwd_copy_invalid_spec","invalid spec");
        for(int mode:{4,5}){const auto bad=run(mode);require(bad.reason=="cwd_copy_precondition_shape" && !bad.cwd_copy.continued && bad.cwd_query.verified,"bad shape advanced");}
        const auto query_shape=run(6);require(query_shape.reason=="cwd_query_precondition_shape" && !query_shape.cwd_copy.armed,"query shape advanced");
        require(run(7).reason=="loader_event_limit","event budget");
        const auto legacy=run(8);require(legacy.reason=="cwd_query_verified" && legacy.cwd_query.verified && !legacy.cwd_copy.armed,"legacy query advanced");
        require(run(9).cwd_copy.verified,"owner recovery");
        const auto mismatch=run(10);require(mismatch.reason=="cwd_query_directory_mismatch" && mismatch.cwd_query.function_returned && !mismatch.cwd_copy.armed,"different cwd copied");
        const auto acquire_only=run(11);require(acquire_only.reason=="cwd_acquire_verified" && !acquire_only.cwd_query.armed && !acquire_only.cwd_copy.armed,"legacy acquire advanced");
        {
            const auto held=CreateEventA(nullptr,TRUE,FALSE,"Local\\SAEX.CwdCopyFixture.v1");require(held,"held event");
            const auto v=run();const auto state=WaitForSingleObject(held,0);CloseHandle(held);
            require(v.reason=="instance_existing_detected" && !v.cwd_copy.armed && state==WAIT_TIMEOUT,"existing instance advanced");
        }
        for(const auto path:{argv[1]}) {
            ...
            if (waited!=WAIT_OBJECT_0) { TerminateProcess(process.hProcess,99);WaitForSingleObject(process.hProcess,5000); }
            const auto read=GetExitCodeProcess(process.hProcess,&code);CloseHandle(process.hThread);CloseHandle(process.hProcess);
            require(waited==WAIT_OBJECT_0 && read && code==92 && markers.exists(L".event-app-called"),"post-copy canary insensitive");
        }
        const auto scenarios=cases;DWORD before{},after{};
        require(GetProcessHandleCount(GetCurrentProcess(),&before)!=0,"handles before");
        for (unsigned i=0;i<12;++i) require(run().cwd_copy.verified,"warm boundary");
        require(GetProcessHandleCount(GetCurrentProcess(),&after)!=0 && before==after,"handle leak");
        std::cout<<"PASS cwd copy: "<<scenarios<<" scenarios, one post-copy canary control and 12 warm cycles; handles "<<before<<" -> "<<after<<'\n';return 0;
