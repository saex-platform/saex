...
int wmain(int argc,wchar_t** argv) {
    ...
    try {
        ...
        auto run=[&](int mode=0) {
            ...
            instance.name="Local\\SAEX.CdStreamTablesFixture.v1"; instance.name_rva=exe.symbol("instance_name");
            ...
            if(mode==1)stream_spec.target_rva=0;
            if(mode==2)stream_spec.names_rva+=4;
            if(mode==3)stream_spec.disk_iat_rva+=1;
            if(mode==4){stream_spec.handles_rva+=4;stream_spec.names_rva+=4;}
            if(mode==5)stream_spec.disk_iat_rva+=4;
            ...
            if(mode==9){
                LoaderTrace foreign{};std::thread worker([&]{foreign=LoaderObservation::run_cd_stream_tables(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion,ready,stream_spec);});
                worker.join();require(foreign.reason=="loader_owner_thread" && !child.stopped(),"foreign owner");
            }
            const auto result=[&]() -> LoaderTrace {
                if(mode==8)return LoaderObservation::run_file_manager_ready(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion,ready);
                return LoaderObservation::run_cd_stream_tables(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion,ready,stream_spec,limits);
            }();
            std::cout<<"case="<<cases<<" mode="<<mode<<" reason="<<result.reason<<" stage="<<result.cd_stream_tables.stage<<'\n';
        ...
        const auto normal=run();const auto& n=normal.cd_stream_tables;
        require(normal.reason=="cd_stream_tables_verified" && normal.file_manager_ready.verified && n.verified && n.continued &&
            n.shape_valid && n.frame_valid && n.arguments_valid && n.memory_read && n.tables_valid && n.caller_preserved &&
            n.manager_preserved && n.seh_preserved && n.lock_preserved && n.localisation_preserved && n.last_error_preserved && n.stage==2,"normal incomplete");
        for(auto v:n.before)require(v==std::byte{0xa7},"nonzero table fixture missing");
        for(int mode:{1,2,3})require(run(mode).reason=="cd_stream_tables_invalid_spec","invalid spec");
        for(int mode:{4,5}){const auto bad=run(mode);require(bad.reason=="cd_stream_tables_precondition_shape" && !bad.cd_stream_tables.continued && bad.file_manager_ready.verified,"bad shape advanced");}
        const auto bad_query=run(6);require(bad_query.reason=="cwd_query_precondition_shape" && !bad_query.cd_stream_tables.armed,"query shape advanced");
        require(run(7).reason=="loader_event_limit","event budget");
        const auto legacy=run(8);require(legacy.reason=="file_manager_ready_verified" && !legacy.cd_stream_tables.armed,"legacy ready advanced");
        require(run(9).cd_stream_tables.verified,"owner recovery");
        const auto mismatch=run(10);require(mismatch.reason=="cwd_query_directory_mismatch" && !mismatch.cd_stream_tables.armed,"wrong cwd advanced");
        {
            const auto held=CreateEventA(nullptr,TRUE,FALSE,"Local\\SAEX.CdStreamTablesFixture.v1");require(held,"held event");
            const auto v=run();const auto state=WaitForSingleObject(held,0);CloseHandle(held);
            require(v.reason=="instance_existing_detected" && !v.cd_stream_tables.armed && state==WAIT_TIMEOUT,"existing instance advanced");
        }
        for(const auto path:{argv[1]}) {
            ...
            require(waited==WAIT_OBJECT_0 && read && code==92 && markers.exists(L".event-app-called"),"disk-call canary insensitive");
        }
        const auto scenarios=cases;DWORD before{},after{};
        require(GetProcessHandleCount(GetCurrentProcess(),&before)!=0,"handles before");
        for (unsigned i=0;i<12;++i) require(run().cd_stream_tables.verified,"warm boundary");
        require(GetProcessHandleCount(GetCurrentProcess(),&after)!=0 && before==after,"handle leak");
        std::cout<<"PASS cd stream tables: "<<scenarios<<" scenarios, one disk-call canary control and 12 warm cycles; handles "<<before<<" -> "<<after<<'\n';return 0;
