...
import cwd_acquire_policy as tool
class PolicyTests(unittest.TestCase):
    ...
    def test_identity(self):
        for k,v in [('schemaVersion',True),('extra',0),('id','/'),('stage','directory'),('reviewDocument','no.md'),('lockPolicySha256','0'*64)]:self.reject(lambda p:p.update({k:v}))
        self.reject(lambda p:p['systemModule'].update(sha256='0'*64));self.reject(lambda p:p['systemModule'].update(bytes=True))
    def test_ranges(self):
        for v in (True,0,4095,2**32,self.p['iatRva']+1):self.reject(lambda p:p.update(iatRva=v))
        for k,v in [('rva',True),('rva',2**32),('preferredBase',1),('highlowOffsets',[0]),('extra',1)]:self.reject(lambda p:p['function'].update({k:v}))
    def test_body_chain(self):
        for i in range(11):
            b=bytearray.fromhex(self.p['bodyHex']);b[i]^=1;self.reject(lambda p:p.update(bodyHex=b.hex()))
        self.reject(lambda p:p.update(returnPrefixHex='00'*16))
        self.reject(lambda p:p['function'].update(prefixHex='zz'*20))
