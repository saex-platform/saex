...
int wmain(int argc,wchar_t** argv) {
    ...
    try {
        ...
        auto run=[&](int mode=0) {
            ...
            instance.name="Local\\SAEX.CdStreamDiskFixture.v1"; instance.name_rva=exe.symbol("instance_name");
            ...
            if(mode==1)disk.flags_rva=0;
            if(mode==2)disk.allocator_rva=stream_spec.target_rva;
            if(mode==3)disk.thunk_slot_rva+=1;
            if(mode==4)disk.flags_rva+=4;
            if(mode==5)disk.allocator_rva+=1;
            if(mode==6)disk.implementation.prefix[0]^=std::byte{1};
            if(mode==7)disk.thunk_rva+=1;
            if(mode==8)disk.thunk_slot_rva+=4;
            if(mode==9)query.expected_directory="C:\\wrong-directory";
            LoaderLimits limits{};if(mode==10)limits.events=1;
            if(mode==12){
                auto foreign=std::make_unique<LoaderTrace>();std::thread worker([&]{*foreign=LoaderObservation::run_cd_stream_disk(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion,ready,stream_spec,disk);});
            ...
            *result=[&]() -> LoaderTrace {
                if(mode==11)return LoaderObservation::run_cd_stream_tables(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion,ready,stream_spec);
                return LoaderObservation::run_cd_stream_disk(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion,ready,stream_spec,disk,limits);
            }();
            std::cout<<"case="<<cases<<" mode="<<mode<<" reason="<<result->reason<<" stage="<<result->cd_stream_disk.stage<<'\n';
        ...
        const auto normal=run();const auto& n=normal->cd_stream_disk;
        require(normal->reason=="cd_stream_disk_verified" && normal->cd_stream_tables.verified && n.verified && n.stage==4 &&
            n.query_succeeded && n.api_returned && n.geometry_valid && n.globals_valid && n.arguments_valid && n.frame_valid &&
            n.tables_preserved && n.caller_preserved && n.manager_preserved && n.seh_preserved && n.lock_preserved &&
            n.localisation_preserved && n.last_error_read,"normal incomplete");
        require(n.globals_before==std::array<std::uint32_t,5>{11,22,33,44,55},"sentinel globals");
        for(int mode:{1,2,3})require(run(mode)->reason=="cd_stream_disk_invalid_spec","invalid disk spec");
        for(int mode:{4,5,6,7,8}){const auto v=run(mode);require(v->reason=="cd_stream_disk_precondition_shape" && !v->cd_stream_disk.continued && v->cd_stream_tables.verified,"bad shape advanced");}
        require(run(9)->reason=="cwd_query_directory_mismatch","wrong cwd");
        require(run(10)->reason=="loader_event_limit","event budget");
        const auto legacy=run(11);require(legacy->reason=="cd_stream_tables_verified" && !legacy->cd_stream_disk.armed,"legacy advanced");
        require(run(12)->cd_stream_disk.verified,"owner recovery");
        {
            const auto held=CreateEventA(nullptr,TRUE,FALSE,"Local\\SAEX.CdStreamDiskFixture.v1");require(held,"held event");
            const auto v=run();const auto state=WaitForSingleObject(held,0);CloseHandle(held);
            require(v->reason=="instance_existing_detected" && !v->cd_stream_disk.armed && state==WAIT_TIMEOUT,"existing instance advanced");
        }
        for(const auto path:{argv[1]}) {
            ...
            require(waited==WAIT_OBJECT_0 && read && code==92 && markers.exists(L".event-app-called"),"allocation-call canary insensitive");
        }
        const auto scenarios=cases;DWORD before{},after{};
        require(GetProcessHandleCount(GetCurrentProcess(),&before)!=0,"handles before");
        for (unsigned i=0;i<12;++i) require(run()->cd_stream_disk.verified,"warm boundary");
        require(GetProcessHandleCount(GetCurrentProcess(),&after)!=0 && before==after,"handle leak");
        std::cout<<"PASS cd stream disk: "<<scenarios<<" scenarios, one allocation-call canary control and 12 warm cycles; handles "<<before<<" -> "<<after<<'\n';return 0;
