...
int wmain(int argc,wchar_t** argv) {
    ...
    try {
        ...
        auto run=[&](int variant,int mode=0) {
            ...
            if (mode==9) {
                LoaderTrace foreign{}; std::thread thread([&] { foreign=LoaderObservation::run_to_platform_startup(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform); });
            ...
            const auto result=mode==8 ? LoaderObservation::run_to_application_entry(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app)
                : LoaderObservation::run_to_platform_startup(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,limits);
        ...
        require(normal.reason=="platform_startup_boundary_verified" && s.verified && s.continued && s.call_reached &&
        ...
        require(run(0,9).reason=="platform_startup_boundary_verified","owner recovery");
        {
        ...
        for (unsigned i=0;i<12;++i) require(run(0).platform_startup.verified,"warm return");
        require(GetProcessHandleCount(GetCurrentProcess(),&after)!=0 && before==after,"handle leak");
        std::cout<<"PASS platform startup: "<<scenarios<<" scenarios, canary control and 12 warm cycles; handles "<<before<<" -> "<<after<<'\n';
        return 0;
