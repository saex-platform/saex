...
int wmain(int argc,wchar_t** argv) {
    if (argc!=8) return 2; // Full, empty, readonly fixtures; proxy, codec and ASI pins.
    try {
        ...
        unsigned cases{};
        auto run=[&](int mode=0) {
            ++cases; const auto path=argv[mode==10?6:mode==11?7:1]; Markers markers(path); Fixture exe(path);
            ...
            instance.name="Local\\SAEX.CwdLockFixture.v1"; instance.name_rva=exe.symbol("instance_name");
            ...
            if(mode==1)lock.table_rva=0;
            if(mode==2)lock.selector.target_prefix[0]^=std::byte{1};
            if(mode==3)lock.selector.call[1]^=std::byte{1};
            if(mode==4)lock.stop_prefix[15]^=std::byte{1};
            if(mode==5){lock.table_rva+=4;require(cwd_lock_code(lock,selector),"mutated lock code");std::copy_n(selector.begin(),16,lock.selector.target_prefix.begin());}
            if(mode==6)lock.stop_prefix[1]^=std::byte{1};
            LoaderLimits limits{};if(mode==7)limits.events=1;
            if(mode==9){
                LoaderTrace foreign{};std::thread worker([&]{foreign=LoaderObservation::run_cwd_lock(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock);});
                worker.join();require(foreign.reason=="loader_owner_thread" && !child.stopped(),"foreign owner");
            }
            const auto result=[&]() -> LoaderTrace {
                if(mode==8)return LoaderObservation::run_cwd_seh(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh);
                return LoaderObservation::run_cwd_lock(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,limits);
            }();
            std::cout<<"case="<<cases<<" mode="<<mode<<" reason="<<result.reason<<" stage="<<result.cwd_lock.stage<<'\n';
        ...
        const auto normal=run();const auto& n=normal.cwd_lock;
        require(normal.reason=="cwd_lock_verified" && normal.cwd_seh.verified && normal.file_manager_entry.verified &&
            n.verified && n.continued && n.entry_reached && n.comparison_reached && n.shape_valid && n.frame_valid &&
            n.slot_read && n.slot_preserved && n.memory_valid && n.seh_preserved && n.prior_record_preserved && n.caller_preserved && n.buffer_preserved && n.localisation_preserved && n.stage==2,"normal incomplete");
        require(n.slot_value==0x12345678 && n.stack_after[7]==7 && n.stack_after[4]==0,"selector proof missing");
        for(int mode:{1,2,3,6})require(run(mode).reason=="cwd_lock_invalid_spec","invalid spec");
        for(int mode:{4,5}){const auto bad=run(mode);require(bad.reason=="cwd_lock_precondition_shape" && !bad.cwd_lock.continued && bad.cwd_seh.verified,"bad shape advanced");}
        require(run(7).reason=="loader_event_limit","event budget");
        const auto legacy=run(8);require(legacy.reason=="cwd_seh_verified" && legacy.cwd_seh.verified && !legacy.cwd_lock.armed,"legacy advanced");
        require(run(9).cwd_lock.verified,"owner recovery");
        const auto empty=run(10);require(empty.reason=="cwd_lock_verified" && empty.cwd_lock.slot_value==0 && empty.cwd_lock.verified,"empty branch escaped");
        const auto readonly_slot=run(11);require(readonly_slot.reason=="cwd_lock_precondition_slot" && readonly_slot.cwd_seh.verified && !readonly_slot.cwd_lock.continued,"readonly slot advanced");
        {
            const auto held=CreateEventA(nullptr,TRUE,FALSE,"Local\\SAEX.CwdLockFixture.v1");require(held,"held event");
            const auto v=run();const auto state=WaitForSingleObject(held,0);CloseHandle(held);
            require(v.reason=="instance_existing_detected" && !v.cwd_lock.armed && state==WAIT_TIMEOUT,"existing instance advanced");
        }
        for(const auto path:{argv[1],argv[6]}) {
            ...
            require(waited==WAIT_OBJECT_0 && read && code==92 && markers.exists(L".event-app-called"),"post-comparison canary insensitive");
        }
        const auto scenarios=cases;DWORD before{},after{};
        require(GetProcessHandleCount(GetCurrentProcess(),&before)!=0,"handles before");
        for (unsigned i=0;i<12;++i) require(run().cwd_lock.verified,"warm boundary");
        require(GetProcessHandleCount(GetCurrentProcess(),&after)!=0 && before==after,"handle leak");
        std::cout<<"PASS cwd lock: "<<scenarios<<" scenarios, two post-comparison canary controls and 12 warm cycles; handles "<<before<<" -> "<<after<<'\n';return 0;
