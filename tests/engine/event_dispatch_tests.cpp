...
int wmain(int argc,wchar_t** argv) {
    ...
    try {
        ...
        auto run=[&](int mode=0) {
            ...
            instance.name="Local\\SAEX.EventDispatchFixture.v1"; instance.name_rva=exe.symbol("instance_name");
            ...
            if (mode==1) dispatch.dispatcher.call_rva++;
            if (mode==2) dispatch.dispatcher.target_prefix[0]^=std::byte{1};
            if (mode==3) dispatch.application.call_rva++;
            if (mode==4) dispatch.application_prefix[20]^=std::byte{1};
            if (mode==5) {
                dispatch.application_prefix[0]^=std::byte{1};
                dispatch.application.target_prefix[0]=dispatch.application_prefix[0];
            }
            if (mode==6) { // Consistent rel32 metadata, wrong runtime target.
                ++dispatch.application.target_rva;
                DWORD raw{};std::memcpy(&raw,dispatch.application.call.data()+1,4);++raw;
                std::memcpy(dispatch.application.call.data()+1,&raw,4);
                std::copy_n(dispatch.application.call.begin(),4,dispatch.dispatcher.target_prefix.begin()+12);
            }
            LoaderLimits limits{};if (mode==7) limits.events=1;
            if (mode==9) {
                LoaderTrace foreign{};std::thread worker([&] { foreign=LoaderObservation::run_event_dispatch(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch); });
                worker.join();require(foreign.reason=="loader_owner_thread" && !child.stopped(),"foreign owner");
            }
            const auto result=[&]() -> LoaderTrace {
                if (mode==8) return LoaderObservation::run_instance_startup(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance);
                return LoaderObservation::run_event_dispatch(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,limits);
            }();
            std::cout<<"case="<<cases<<" mode="<<mode<<" reason="<<result.reason<<" stage="<<result.event_dispatch.stage<<'\n';
        ...
        const auto normal=run();const auto& n=normal.event_dispatch;
        require(normal.reason=="event_dispatch_boundary_verified" && normal.instance_startup.verified && n.verified &&
            n.continued && n.call_reached && n.entry_reached && n.application_call_reached && n.shape_valid && n.frame_valid && n.stack_preserved && n.stage==3,"normal incomplete");
        for (int mode:{1,2,3}) require(run(mode).reason=="event_dispatch_invalid_spec","invalid spec");
        for (int mode:{4,5,6}) {
            const auto bad=run(mode);require(bad.reason=="event_dispatch_precondition_shape" && !bad.event_dispatch.continued,"bad shape advanced");
        }
        require(run(7).reason=="loader_event_limit","event budget");
        const auto legacy=run(8);require(legacy.instance_startup.verified && !legacy.event_dispatch.armed,"legacy advanced");
        require(run(9).event_dispatch.verified,"owner recovery");
        {
            const auto held=CreateEventA(nullptr,TRUE,FALSE,"Local\\SAEX.EventDispatchFixture.v1");require(held,"held event");
            const auto v=run();const auto state=WaitForSingleObject(held,0);CloseHandle(held);
            require(v.reason=="instance_existing_detected" && !v.event_dispatch.armed && state==WAIT_TIMEOUT,"existing instance advanced");
        }
        {
            ...
            require(waited==WAIT_OBJECT_0 && read && code==92 && markers.exists(L".event-app-called"),"application canary insensitive");
        }
        const auto scenarios=cases;DWORD before{},after{};
        require(GetProcessHandleCount(GetCurrentProcess(),&before)!=0,"handles before");
        for (unsigned i=0;i<12;++i) require(run().event_dispatch.verified,"warm boundary");
        require(GetProcessHandleCount(GetCurrentProcess(),&after)!=0 && before==after,"handle leak");
        std::cout<<"PASS event dispatch: "<<scenarios<<" scenarios, application canary control and 12 warm cycles; handles "<<before<<" -> "<<after<<'\n';return 0;
