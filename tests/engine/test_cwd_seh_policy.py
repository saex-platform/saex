...
class PolicyTests(unittest.TestCase):
    ...
    def test_identity(self):
        for k,v in [('schemaVersion',True),('extra',0),('id','/'),('stage','lock'),('reviewDocument','no.md'),('managerPolicySha256','0'*64)]:self.reject(lambda p:p.update({k:v}))
    def test_ranges(self):
        for k in ('scopeRva','handlerRva','cleanupRva'):
            for v in (True,0,4095,2**32,self.p['wrapper']['targetRva']):self.reject(lambda p:p.update({k:v}))
        for key in ('wrapper','prologue'):
            for k,v in [('callRva',True),('targetRva',2**32),('callHex','e800000000'),('extra',1)]:self.reject(lambda p:p[key].update({k:v}))
    def test_body_chain(self):
        for key,n in [('prologueBodyHex',59),('scopeHex',12)]:
            for i in range(n):
                b=bytearray.fromhex(self.p[key]);b[i]^=1;self.reject(lambda p:p.update({key:b.hex()}))
        self.reject(lambda p:p.update(stopPrefixHex='00'*16))
        self.reject(lambda p:p['wrapper'].update(targetPrefixHex='00'*16))
        self.reject(lambda p:p['prologue'].update(callRva=p['prologue']['callRva']+1))
