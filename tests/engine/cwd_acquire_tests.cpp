...
int wmain(int argc,wchar_t** argv) {
    if (argc!=12) return 2; // Full, empty, readonly fixtures; proxy, codec and ASI pins.
    try {
        ...
        unsigned cases{};
        auto run=[&](int mode=0) {
            ++cases; const auto path=argv[mode>=10 && mode<=15?mode-4:1]; Markers markers(path); Fixture exe(path);
            ...
            instance.name="Local\\SAEX.CwdAcquireFixture.v1"; instance.name_rva=exe.symbol("instance_name");
            ...
            if(mode==1)acquire.iat_rva=0;
            if(mode==2)acquire.function.rva=0;
            if(mode==3)acquire.function.highlow_mask=1;
            if(mode==4)acquire.return_prefix[15]^=std::byte{1};
            if(mode==5)acquire.function.prefix[0]^=std::byte{1};
            if(mode==6)acquire.iat_rva+=4;
            LoaderLimits limits{};if(mode==7)limits.events=1;
            if(mode==9){
                LoaderTrace foreign{};std::thread worker([&]{foreign=LoaderObservation::run_cwd_acquire(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire);});
                worker.join();require(foreign.reason=="loader_owner_thread" && !child.stopped(),"foreign owner");
            }
            const auto result=[&]() -> LoaderTrace {
                if(mode==8)return LoaderObservation::run_cwd_lock(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock);
                return LoaderObservation::run_cwd_acquire(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,limits);
            }();
            std::cout<<"case="<<cases<<" mode="<<mode<<" reason="<<result.reason<<" stage="<<result.cwd_acquire.stage<<'\n';
        ...
        const auto normal=run();const auto& n=normal.cwd_acquire;
        require(normal.reason=="cwd_acquire_verified" && normal.cwd_lock.verified && n.verified && n.acquired && n.continued && n.branch_reached && n.call_reached && n.function_entered && n.function_returned && n.selector_returned &&
            n.shape_valid && n.frame_valid && n.object_read && n.object_valid && n.slot_preserved && n.stack_preserved && n.seh_preserved && n.prior_record_preserved && n.caller_preserved && n.buffer_preserved && n.localisation_preserved && n.stage==5,"normal incomplete");
        const auto image=run(15);require(image.cwd_acquire.verified && image.cwd_acquire.object_memory_type==MEM_IMAGE && n.object_memory_type==MEM_PRIVATE,"object origin coverage");
        require(n.object_before[1]==UINT32_MAX && n.object_after[1]==UINT32_MAX-1 && n.object_after[2]==1 && n.object_after[3]==n.thread_id,"ownership proof missing");
        for(int mode:{1,2,3})require(run(mode).reason=="cwd_acquire_invalid_spec","invalid spec");
        for(int mode:{4,5,6,13}){const auto bad=run(mode);require(bad.reason=="cwd_acquire_precondition_shape" && !bad.cwd_acquire.continued && bad.cwd_lock.verified,"bad shape advanced");}
        require(run(7).reason=="loader_event_limit","event budget");
        const auto legacy=run(8);require(legacy.reason=="cwd_lock_verified" && legacy.cwd_lock.verified && !legacy.cwd_acquire.armed,"legacy advanced");
        require(run(9).cwd_acquire.verified,"owner recovery");
        const auto empty=run(10);require(empty.reason=="cwd_acquire_missing_slot" && empty.cwd_lock.verified && !empty.cwd_acquire.armed,"empty branch escaped");
        for(int mode:{11,12,14}){const auto bad=run(mode);require(bad.reason=="cwd_acquire_precondition_object" && bad.cwd_lock.verified && !bad.cwd_acquire.continued,"invalid object advanced");}
        {
            const auto held=CreateEventA(nullptr,TRUE,FALSE,"Local\\SAEX.CwdAcquireFixture.v1");require(held,"held event");
            const auto v=run();const auto state=WaitForSingleObject(held,0);CloseHandle(held);
            require(v.reason=="instance_existing_detected" && !v.cwd_acquire.armed && state==WAIT_TIMEOUT,"existing instance advanced");
        }
        ...
        for (unsigned i=0;i<12;++i) require(run().cwd_acquire.verified,"warm boundary");
        require(GetProcessHandleCount(GetCurrentProcess(),&after)!=0 && before==after,"handle leak");
        std::cout<<"PASS cwd acquire: "<<scenarios<<" scenarios, two post-acquire canary controls and 12 warm cycles; handles "<<before<<" -> "<<after<<'\n';return 0;
