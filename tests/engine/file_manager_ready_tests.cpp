...
int wmain(int argc,wchar_t** argv) {
    ...
    try {
        ...
        auto run=[&](int mode=0) {
            ...
            instance.name="Local\\SAEX.FileManagerReadyFixture.v1"; instance.name_rva=exe.symbol("instance_name");
            ...
            if(mode==1)ready.unlock_rva=0;
            if(mode==2)ready.unlock_rva=manager.buffer_rva;
            if(mode==3)ready.unlock_rva=UINT32_MAX;
            if(mode==4)ready.function.prefix[0]^=std::byte{1};
            if(mode==5)ready.epilogue_rva+=1;
            if(mode==6)query.implementation.prefix[0]^=std::byte{1};
            if(mode==10)query.expected_directory="C:\\wrong-directory";
            LoaderLimits limits{};if(mode==7)limits.events=1;
            if(mode==9){
                LoaderTrace foreign{};std::thread worker([&]{foreign=LoaderObservation::run_file_manager_ready(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion,ready);});
                worker.join();require(foreign.reason=="loader_owner_thread" && !child.stopped(),"foreign owner");
            }
            const auto result=[&]() -> LoaderTrace {
                if(mode==12)return LoaderObservation::run_cwd_copy(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy);
                if(mode==11)return LoaderObservation::run_cwd_acquire(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire);
                if(mode==8)return LoaderObservation::run_cwd_query(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query);
                return LoaderObservation::run_file_manager_ready(child,exe.file.handle(),pins,entry,proxy,startup,codec,binding,asi,tail,crt,app,platform,suppression,instance,dispatch,routing,prelude,manager,seh,lock,acquire,query,copy,completion,ready,limits);
            }();
            std::cout<<"case="<<cases<<" mode="<<mode<<" reason="<<result.reason<<" stage="<<result.file_manager_ready.stage<<'\n';
        ...
        const auto normal=run();const auto& n=normal.file_manager_ready;
        require(normal.reason=="file_manager_ready_verified" && normal.cwd_return.verified && n.verified && n.continued &&
            n.shape_valid && n.frame_valid && n.arguments_valid && n.lock_released && n.seh_removed && n.prior_record_preserved &&
            n.caller_preserved && n.buffer_valid && n.suffix_written && n.wrapper_returned && n.manager_returned &&
            n.localisation_preserved && n.last_error_preserved && n.stage==9,"normal incomplete");
        for(int mode:{1,2,3})require(run(mode).reason=="file_manager_ready_invalid_spec","invalid spec");
        for(int mode:{4,5}){const auto bad=run(mode);require(bad.reason=="file_manager_ready_precondition_shape" && !bad.file_manager_ready.continued && bad.cwd_return.verified,"bad shape advanced");}
        const auto query_shape=run(6);require(query_shape.reason=="cwd_query_precondition_shape" && !query_shape.file_manager_ready.armed,"query shape advanced");
        require(run(7).reason=="loader_event_limit","event budget");
        const auto legacy=run(8);require(legacy.reason=="cwd_query_verified" && legacy.cwd_query.verified && !legacy.file_manager_ready.armed,"legacy query advanced");
        require(run(9).file_manager_ready.verified,"owner recovery");
        const auto mismatch=run(10);require(mismatch.reason=="cwd_query_directory_mismatch" && mismatch.cwd_query.function_returned && !mismatch.file_manager_ready.armed,"different cwd copied");
        const auto acquire_only=run(11);require(acquire_only.reason=="cwd_acquire_verified" && !acquire_only.cwd_query.armed && !acquire_only.file_manager_ready.armed,"legacy acquire advanced");
        const auto copy_only=run(12);require(copy_only.reason=="cwd_copy_verified" && copy_only.cwd_copy.verified && !copy_only.file_manager_ready.armed,"legacy copy advanced");
        {
            const auto held=CreateEventA(nullptr,TRUE,FALSE,"Local\\SAEX.FileManagerReadyFixture.v1");require(held,"held event");
            const auto v=run();const auto state=WaitForSingleObject(held,0);CloseHandle(held);
            require(v.reason=="instance_existing_detected" && !v.file_manager_ready.armed && state==WAIT_TIMEOUT,"existing instance advanced");
        }
        for(const auto path:{argv[1]}) {
            ...
            require(waited==WAIT_OBJECT_0 && read && code==92 && markers.exists(L".event-app-called"),"post-manager canary insensitive");
        }
        const auto scenarios=cases;DWORD before{},after{};
        require(GetProcessHandleCount(GetCurrentProcess(),&before)!=0,"handles before");
        for (unsigned i=0;i<12;++i) require(run().file_manager_ready.verified,"warm boundary");
        require(GetProcessHandleCount(GetCurrentProcess(),&after)!=0 && before==after,"handle leak");
        std::cout<<"PASS file manager ready: "<<scenarios<<" scenarios, one post-manager canary control and 12 warm cycles; handles "<<before<<" -> "<<after<<'\n';return 0;
