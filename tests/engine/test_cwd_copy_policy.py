...
import cwd_copy_policy as tool
class PolicyTests(unittest.TestCase):
    ...
    def test_identity_parent(self):
        for k,v in [('schemaVersion',True),('id','/'),('stage','helper-return'),('queryPolicySha256','1'*64),('extra',0),('reviewDocument','no.md')]:self.reject(lambda p:p.update({k:v}))
    def test_code_ranges_overlap(self):
        for value in (True,0,4095,2**32):self.reject(lambda p:p.update(copierRva=value))
        q=json.loads(self.data[1]);self.reject(lambda p:p.update(copierRva=q['helperRva']))
        self.reject(lambda p:p.update(copierRva=q['cookieRva']))
    def test_every_admitted_byte(self):
        for key in ('checksHex','capacityHex','callHex','entryHex','bodyHex'):
