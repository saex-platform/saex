...
Üçüncü hosted koşuda startup-return/CRT/uygulama/platform başlangıcı geçti; suppression ve ona bağlı sekiz grup `suppression_invalid_spec` / `fixture instance spec` ile reddedildi. Server 2022 kernel32 GetLastError prefix'i normatif `MOV EAX,FS:[34h]; RET` biçimini sağlamaz. Bu davranış kontrolü korunarak x86 GitHub işleri `windows-2025` ortamına taşındı; x64 Server 2022 kapsamı korundu. Gerçek GTA OS/hash izinleri aynı kaldı.
