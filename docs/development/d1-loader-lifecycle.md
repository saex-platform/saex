...
Codec-return, root-first 1–3 beklenen mapping için aktif file ID/hash/boyut, tekillik ve armEvent sonrası yeni LOAD şartı getirir. Önceden aktif dependency reddedilir; DllMain FALSE sonrası unload aynı ledger üzerinden tutulur, lifetime bütçe geri verilmez. [Sözleşme ve doğrulama](d1-codec-return.md).

## Kod 0.1.15 — Codec fonksiyon bağları

Binding fazında root/dependency mapping ID emekliliği terminaldir; bu faz hiçbir yeni LOAD_DLL olayına devam izni vermez. Aynı base ile remap eski mapping kimliğini diriltmez, lifetime budget sıfırlanmaz. [Sözleşme ve kanıt](d1-codec-bindings.md).
...
Frame modu aynı lifetime module/event bütçesini, unload kaydını ve owned child temizliğini kullanır. Üç EXE örneği modül bütçesini artırmaz veya retire olmuş mapping kabul etmez. [Aday ve doğrulama raporu](d1-frame-target.md).
