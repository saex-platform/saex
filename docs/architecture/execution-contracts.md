...
JSON cwdQueryObservation ayrı izin/gözlem alanları taşır: directoryApiAllowed, functionReturned, pathVerified ve verified farklıdır. Ham stack buffer dışarı verilmez. C++ LoaderTrace genişler ve tüketicileri yeniden derlenir; bootstrap C ABI 1 ve ağ/otorite sözleşmesi aynı kalır. [Sözleşme, kaynak ve güncel kanıt](../development/d1-cwd-query.md).
...
Yeni doğal yürütme izni wrapper cleanup, unlock(7), normal SEH epilogue ve CFileMgr dönüşünü tek üst düzey işlemde kapsar. API VOID dönüşü register başarı koduyla yorumlanmaz; aynı kilit nesnesinin boş görüntüsü ve eski FS zinciri readback ile doğrulanır. Helper local kaynağı retired olduğundan daha sonra eski stack snapshot korunumu zorlanmaz. Parent observation kayıtları kendi checkpoint zamanına aittir; final durum `fileManagerReadyObservation` içindedir. [Sözleşme, kullanıcı komutu ve doğrulama](../development/d1-file-manager-ready.md).
